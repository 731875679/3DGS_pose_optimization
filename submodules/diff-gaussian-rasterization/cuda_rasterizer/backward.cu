...
template <typename group_t, typename T>
__device__ void render_cuda_reduce_sum_impl(group_t g, int lane, int i, T* list) {
    reduce_helper(lane, i, list);
}

template <typename group_t, typename T, typename... Lists>
__device__ void render_cuda_reduce_sum_impl(group_t g, int lane, int i, T* list, Lists... lists) {
    reduce_helper(lane, i, list);
    render_cuda_reduce_sum_impl(g, lane, i, lists...);
}
